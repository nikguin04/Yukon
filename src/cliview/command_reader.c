...
Command* MatchCommand(char* cmdinput, size_t len) {
    ...
    for (int a = 0; a < len; a++) {
        ...
        bool allskip = true;
        for (int i = 0; i < COMMAND_COUNT; i++) {
            if (candidate_tail->skip) { continue; }
            allskip = false;
            char candidate_char = candidate_tail->command->input[a];
            if (candidate_char != cmdinput[a]) { //  || candidate_char == (candidate_tail->command->takes_input ? '\n' : ' ')
        ...
        if (allskip == NULL) {printf("COMMAND NOT FOUND!"); return NULL;}
    }
}
